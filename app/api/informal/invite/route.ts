...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await getComposio().tools.execute("GOOGLECALENDAR_CREATE_EVENT", {
      ...
      version: "20260312_00",
      arguments: {
        summary: `Informal Call — ${name}`,
        description: `30-minute informal call with ${name} (${email}).${message ? `\n\nNote: ${message}` : ""}`,
    ...
    // Update CRM lead notes if the lead exists — does NOT change stage or call_date (not a Discovery Call)
    ...
    if (!lead) {
      ...
      if (firstName && lastName) {
        ...
        if (nameMatch) {
          lead = nameMatch
          console.log(`[informal] Matched lead by name (${firstName} ${lastName}), CRM email: ${nameMatch.email}`)
        }
      }
    }

    if (lead) {
      const marker = `[INFORMAL|${startDT.toISOString()}|${meetLink ?? ""}]`;
      ...
      console.warn(`[informal] No CRM lead found for email: ${normalizedEmail}`);
    }
    ...
    if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
      ...
      const leadBody = `Bonjour ${name},\n\nVotre call informel de 30 minutes avec Clément Guiraud est confirmé.\n\n📅 ${dateFR} à ${time} (heure de Paris)\n\n${meetLine}\n\nÀ très vite,\nClément`;
      const orgBody = `Informal call réservé — ${name} (${email})\n\n📅 ${dateFR} à ${time}${phone ? `\n📱 ${phone}` : ""}${meetLink ? `\n🎥 ${meetLink}` : ""}${ev?.htmlLink ? `\n📆 ${ev.htmlLink}` : ""}`;

      const leadSubject = `✓ Call confirmé — ${dateFR} à ${time}`;
      const orgSubject = `📞 Informal : ${name} — ${dateFR} à ${time}`;

      const icsContent = buildBlockICS(startDT, meetLink ?? null, `informal-${Date.now()}@koja`);

      await Promise.all([
        transporter.sendMail({ from, to: email, subject: leadSubject, html: buildEmailHtml(leadBody, ctx) })
          .then(() => logEmail({ to_email: email, subject: leadSubject, status: "success", source: "informal" }))
          .catch((e) => logEmail({ to_email: email, subject: leadSubject, status: "error", error: String(e), source: "informal" })),
        transporter.sendMail({ from, to: ORGANIZER_EMAIL, subject: orgSubject, html: buildEmailHtml(orgBody, ctx) })
          .then(() => logEmail({ to_email: ORGANIZER_EMAIL, subject: orgSubject, status: "success", source: "informal" }))
          .catch((e) => logEmail({ to_email: ORGANIZER_EMAIL, subject: orgSubject, status: "error", error: String(e), source: "informal" })),
    ...
    const reminderSteps = [
      {
        ...
        subject: interpolate("Rappel : votre call informel demain à {{time}}", reminderCtx),
        body: `Bonjour {{name}},\n\nRappel : votre call informel avec Clément est demain.\n\n📅 {{date}} à {{time}} (heure de Paris)\n\n{{meetLink}}\n\nÀ demain,\nClément`,
      },
      {
        offsetMs: 60 * 60 * 1000,
        subject: interpolate("Dans 1 heure : votre call informel à {{time}}", reminderCtx),
        body: `Bonjour {{name}},\n\nVotre call informel commence dans 1 heure.\n\n📅 {{date}} à {{time}} (heure de Paris)\n\n{{meetLink}}\n\nÀ tout à l'heure,\nClément`,
      },
    ];
    ...
    for (const step of reminderSteps) {
      ...
      for (const toEmail of reminderEmails) {
        ...
        if (reminderError) console.error("[informal] reminder insert failed:", reminderError.message);
      }
    }
