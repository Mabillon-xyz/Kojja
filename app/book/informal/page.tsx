...
function InformalBooking() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      const res = await fetch('/api/informal/invite', {
        method: 'POST',
  ...
  const LeftPanel = (
    <div className="w-full md:w-56 flex-shrink-0 border-b md:border-b-0 md:border-r border-gray-100 p-6 md:p-8 flex flex-col gap-5">
      <div className="w-11 h-11 rounded-full bg-emerald-600 flex items-center justify-center text-white font-semibold text-base select-none">
        CG
      </div>
      <div>
        <p className="text-xs text-gray-400 mb-0.5">Clément Guiraud</p>
        <h1 className="text-lg font-semibold text-gray-900">Informal Call</h1>
      </div>
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row overflow-hidden w-full max-w-2xl">
          ...
          <div className="flex-1 p-8 flex flex-col items-center justify-center text-center gap-4">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Call confirmé !</h2>
            <p className="text-sm text-gray-500">
              Votre call informel avec Clément est confirmé.<br />
                      ...
                      <button
                        ...
                        className={`w-9 h-9 rounded-full text-sm transition-all font-medium ${
                          isSelected
                            ...
                            : 'text-gray-800 hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer'
                        }`}
                      >
                        {date.getDate()}
                      </button>
                    ...
                    <>
                      ...
                      {selectedTime && (
                        <button
                          ...
                          className="mt-4 w-full py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors"
                        >
                          Continuer →
                        </button>
                      )}
          ...
          {step === 'form' && selectedDate && selectedTime && (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  {submitting ? 'Réservation en cours…' : 'Confirmer le call'}
                </button>
              </form>
            </>
          )}
